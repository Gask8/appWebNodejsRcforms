module.exports = app => {
	...
	//new
  router.get('/evaluados/new', (req,res)=>{
	  var vsession = req.session;
	  res.render('evaluado/new',{vsession})
  });
	
// Create new
  app.post("/evaluados", async(req, res)=>{
	  const data = req.body;
	  var evaluado;
  ...
  // Retrieve all
  router.get("/evaluados", async(req, res)=>{
  ...
  router.get("/evaluados/:evaluadoId", async(req, res)=>{
  ...
  app.delete("/evaluados/:evaluadoId", async(req, res)=>{
